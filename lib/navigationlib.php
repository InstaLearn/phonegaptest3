    ...
    public function initialise() {
        ...
        if (get_home_page() == HOMEPAGE_SITE) {
            ...
            if (isloggedin() && !isguestuser()) {  // Makes no sense if you aren't logged in
                //custom change : author: Dinesh: To remove my home link from home page.
				//$this->rootnodes['home'] = $this->add(get_string('myhome'), new moodle_url('/my/'), self::TYPE_SETTING, null, 'home');
				if(is_siteadmin()){
					$this->rootnodes['mycourses'] = $this->add(get_string('mycourses'), new moodle_url('/course/index.php'), self::TYPE_ROOTNODE, null, 'mycourses');
					
				}
				else{
					$this->rootnodes['mycourses'] = $this->add(get_string('mycourses'), new moodle_url('/#'), self::TYPE_ROOTNODE, null, 'mycourses');
				}
			}
			else{
				$this->rootnodes['mycourses'] = $this->add(get_string('mycourses'), new moodle_url('/course/index.php'), self::TYPE_ROOTNODE, null, 'mycourses');
        ...
        foreach ($this->rootnodes as $node) {
            ...
            if (($node->key !== 'home' && !$node->has_children() && !$node->isactive) || (strcmp($node->key, 'myprofile') == 0 ) || (strcmp($node->key, 'site') == 0 ) ) {
                $node->remove();
            }
